...
import models.ResNet_new as new_models
...
def get_model_and_dataset(args:Namespace)->tuple:
    ...
    if hasattr(new_models, args.model.name):
        model = getattr(new_models, args.model.name)(args)
    else:
...
def Normal_Mode():
    ...
    opt.add_argument('--model', type=str, default='ResNet_ALG_AmbiLatent', help='model name')
    ...
    opt.add_argument('--epochs', type=int, default=None, help='override training epochs')
    opt.add_argument('--batch_size', type=int, default=None, help='override batch size')
    opt.add_argument('--lr', type=float, default=None, help='override learning rate')
    opt = opt.parse_args()
    set_seed(opt.seed)
    ...
    if opt.epochs is not None:
        ...
        args.train.rt_start = min(args.train.rt_start, opt.epochs)
        args.scheduler.total_epoch = args.train.rt_start
        args.rt_scheduler.total_epoch = max(args.train.epochs - args.train.rt_start, 1)
    if opt.batch_size is not None:
        args.train.batch_size = opt.batch_size
    if opt.lr is not None:
        args.train.lr = opt.lr
        args.scheduler.base_lr = opt.lr
    main(args)
