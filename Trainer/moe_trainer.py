...
class MoE_Trainer(Default_Trainer):
    def __init__(self,args,model,dataset,device):
        ...
        if self.args.dataset.loader is not None:
            ...
            if isinstance(self.dataset['train'].targets, list):
                train_labels = torch.tensor(self.dataset['train'].targets, dtype=torch.int64)
            elif isinstance(self.dataset['train'].targets, torch.Tensor):
                train_labels = self.dataset['train'].targets.long()
            elif isinstance(self.dataset['train'].targets,np.ndarray):
                train_labels = torch.tensor(self.dataset['train'].targets, dtype=torch.int64)
            else:
                assert "Not support data type"
            # 判断targets是否是one-hot编码
        ...
        #     warmup_lr=self.args.train.lr * 0.15 if hasattr(self.args.train,'lr_rt') else self.args.train.lr * 0.1
        # )
        ...
        #     eta_min=0.0,
        #     warmup_epochs=0, #TODO：ori 5
        #     base_lr=self.args.train.lr_rt if hasattr(self.args.train,'lr_rt') else self.args.train.lr,
        #     warmup_lr=self.args.train.lr_rt * 0.1 if hasattr(self.args.train,'lr_rt') else self.args.train.lr * 0.1
        # )
        ...
        self.rt_scheduler = lr_scheduler.CosineAnnealingLR(self.rt_optimizer, 
                                                           T_max=self.args.train.epochs - self.args.train.rt_start, 
                                                           eta_min=0, last_epoch=-1)
        ...
        if args.train.use_swanlab and swanlab is not None:
            swanlab.init(project=args.task, name=args.train.save_name)
            # swanlab.config.update(args.train)
    ...
    def train_one_epoch(self, epoch):
        ...
        with tqdm(self.loader['train'], desc=f"Epoch {epoch}", unit='batch') as pbar:
            for i, data in enumerate(pbar):
                ...
                #     grads = []
                #     for j in range(3):
                #         grad= self.model.layer3s[j][-1].conv2.weight.grad
                #         # grad = self.model.classifiers[j].weight.grad
                #         grads.append(grad.norm())
                            
                #     gradients_experts.append(torch.stack(grads))
                #     exp_loss.append((torch.stack(outputs['exp_loss'])))
                ...
                if i % self.args.train.log_interval == 0:
                    ...
                    if self.args.train.use_swanlab and swanlab is not None:
                        swanlab.log({'batch': epoch * len(self.loader['train']) + i, 'training loss': loss.item()})
        if epoch >= self.args.train.rt_start:
            self.rt_scheduler.step()
        else:
            self.scheduler.step()
        # if not epoch >= self.args.train.rt_start:
        #     gradients_experts = torch.stack(gradients_experts)
        #     self.gradients_experts = torch.mean(gradients_experts,dim=0)
        #     exp_loss = torch.stack(exp_loss)
        #     self.exp_loss = torch.mean(exp_loss,dim=0)
        return loss.item()
    ...
    def _eval(self,mode='val',epoch=0):
        ...
        from metrics.metrics import compute_hmt_acc
        hmt_acc = compute_hmt_acc(logits_list, targets_list, self.args.label_dis, head_thr=128, mid_thr=65, num_classes=len(self.args.label_dis))
        ...
        #     import csv
        #     with open(f"{self.save_path}/{mode}_val_acc.csv", 'a') as f:
        #         writer = csv.writer(f)
        ...
        #         with open(f"{self.save_path}/{mode}_exp_loss.csv", 'a') as f:
        #             writer = csv.writer(f)
        #             writer.writerow([loss.item() for loss in self.exp_loss])
        self.model.on_eval_end()
        results = {'loss': {'ce':losses.avg}, 'metric': {'acc1': top1.avg.item(), 'acc5': top5.avg.item()}} 
        # 合并results和hmt_acc（都是字典）
        results['metric'].update(hmt_acc)
        return results
    ...
    def train(self):
        ...
        for epoch in range(self.start_epoch,self.args.train.epochs):
            if epoch >= self.args.train.rt_start:
                for name, param in self.model.named_parameters(): #TODO:这是用glmc resnet临时改的，嵌套model，后面该回去
                    if "rt_classifiers" not in name and "ssm" not in name:
        ...
        if hasattr(self.args.train,'use_wandb') and self.args.train.use_wandb:
            wandb.finish()
        if hasattr(self.args.train,'use_swanlab') and self.args.train.use_swanlab and swanlab is not None:
            swanlab.finish()
        return return_dict
    ...
    def test(self):
        ...
        with torch.no_grad():
            with tqdm(loader, desc=f"Evaluating {mode}", unit='batch') as pbar:
                for _,data in enumerate(pbar):
                    ...
                    top5.update(outputs['acc5'], data['image'].size(0))
                    # exp_acc.append(outputs['exp_acc'])
                    # predictions.append(torch.stack(outputs['pred'],dim=1))
                    # labels.append(torch.tensor(data['label']))
                    # features.append(torch.stack(outputs['feature'],dim=1))
                    predictions.append(outputs['pred'].cpu())
                    labels.append(data['label'].cpu())
                    features.append(outputs['feature'].cpu())
                    alpha.append(outputs['alpha'].cpu())
                # exp_acc = torch.stack(exp_acc)
                # exp_acc = torch.mean(exp_acc,dim=0)
        ...
        from metrics.metrics import compute_hmt_acc
        hmt_acc = compute_hmt_acc(predictions, labels, self.args.label_dis, head_thr=128, mid_thr=65, num_classes=len(self.args.label_dis))
        print(hmt_acc)
        # 创建一个pth文件，保存模型的预测结果
        torch.save({'class_freq':torch.tensor(self.args.label_dis),
            'predictions':predictions,
                    'labels':labels,
                    'features':features,
                    'alpha':alpha,
                    },
                    f"{self.save_path}/predictions.pth")
        # if hasattr(self,'save_path'):
        #     import csv
